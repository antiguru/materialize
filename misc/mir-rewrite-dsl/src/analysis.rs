...
//!
//! The driver lives on [`crate::egraph::EGraph`] as `run_analysis`. The same
...
impl Analysis for Monotonic {
    ...
    fn make(&self, node: &ENode, get: &dyn Fn(Id) -> bool, _arity: &dyn Fn(Id) -> usize) -> bool {
        match node {
            // A retraction (Negate) or an aggregate that can move down (Reduce)
            // is not insert-only.
            ENode::Negate { .. } | ENode::Reduce { .. } => false,
...
impl Analysis for Keys {
    ...
    fn make(&self, node: &ENode, get: &dyn Fn(Id) -> KeySet, arity: &dyn Fn(Id) -> usize) -> KeySet {
        match node {
...
fn join_keys(
    ...
    let parts: Vec<(KeySet, usize)> = inputs
        .iter()
        .map(|&i| (keys_of(i), arity_of(i)))
        .collect();
...
// --- the same analyses over a plain `Rel` tree (for the greedy optimizer and
//     for recursive bindings) — each is just an instance of `RecAnalysis` ----
    ...
    fn reduce_establishes_a_key_preserved_by_filter() {
        ...
        let plan = Rel::Filter {
            ...
            input: Box::new(Rel::Reduce {
                ...
                group_key: vec![Scalar::new("#0", [0usize])],
                aggregates: vec![],
            }),
        };
