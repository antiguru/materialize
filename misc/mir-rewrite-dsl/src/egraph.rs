...
use crate::analysis::{is_superkey, Analysis, KeySet, Keys, LocalFacts, Monotonic, NonNeg};
use crate::cost::{Cost, CostModel};
use crate::dsl::*;
use crate::ir::{Rel, Scalar};
use crate::matcher::{eval_ixexpr, eval_pexpr, Payload};
...
pub enum ENode {
    ...
    /// peels the binding scopes around them).
    LocalGet {
        id: usize,
        arity: usize,
    },
...
impl ENode {
    ...
    pub fn children(&self) -> Vec<Id> {
        match self {
            ENode::Constant { .. } | ENode::Get { .. } | ENode::LocalGet { .. } => vec![],
            ENode::Project { input, .. }
            | ENode::Map { input, .. }
            | ENode::Filter { input, .. }
            | ENode::Reduce { input, .. }
    ...
    fn map_children(&self, f: impl Fn(Id) -> Id) -> ENode {
        ...
        match &mut n {
            ENode::Constant { .. } | ENode::Get { .. } | ENode::LocalGet { .. } => {}
            ENode::Project { input, .. }
            | ENode::Map { input, .. }
            | ENode::Filter { input, .. }
            | ENode::Reduce { input, .. }
...
impl EGraph {
    ...
    pub fn add_rel(&mut self, rel: &Rel) -> Id {
        let node = match rel {
            ...
            // fragment.
            Rel::LocalGet { id, arity } => ENode::LocalGet {
                id: *id,
                arity: *arity,
            },
            ...
            for node in nodes {
                let a = match node {
                    ...
                    ENode::Map { input, scalars } => {
                        self.arity_guarded(*input, visiting).map(|a| a + scalars.len())
                    }
                    ENode::Filter { input, .. }
                    | ENode::Negate { input }
...
impl Compiler {
    ...
    fn compile(&mut self, pat: &Pat) -> VarId {
        match pat {
            ...
            Pat::Negate(input) | Pat::Threshold(input) => self.unary(pat, input),
...
fn read_payloads(pat: &Pat, node: &ENode, b: &mut EBindings) -> Option<()> {
    match (pat, node) {
        ...
        (Pat::Negate(_), ENode::Negate { .. }) | (Pat::Threshold(_), ENode::Threshold { .. }) => {}
        (
            Pat::Join {
                equivalences,
                inputs,
...
impl EGraph {
    ...
    fn check_conds(&self, conds: &[Cond], b: &EBindings, an: &Analyses) -> bool {
        ...
        conds.iter().all(|c| match c {
            ...
            Cond::AllTrue { payload } => b
                ...
                .is_some_and(|xs| xs.iter().all(|s| crate::scalar::folds_true(&s.text))),
            Cond::AnyFalse { payload } => b
                .payloads
                .get(payload)
                .and_then(|p| p.scalars())
                .is_some_and(|xs| xs.iter().any(|s| crate::scalar::folds_false(&s.text))),
            Cond::AllColumns { payload } => b
                .payloads
                .get(payload)
                .and_then(|p| p.scalars())
                .is_some_and(|xs| xs.iter().all(|s| crate::scalar::folds_col(&s.text).is_some())),
        })
    }
    ...
    /// to a fixpoint. (The e-graphs we build are acyclic, so this converges in
    /// at most the depth of the e-graph.)
                ...
                for node in nodes {
                    if let Some(rel) = self.build_rel(node, &best) {
                        ...
                        let better = match best.get(&id) {
                            None => true,
                            Some((bc, br)) => c.lt(bc) || (c == *bc && rel < *br),
                        };
                        if better {
    ...
    fn build_rel(&self, node: &ENode, best: &HashMap<Id, (Cost, Rel)>) -> Option<Rel> {
        ...
        Some(match node {
            ...
            ENode::LocalGet { id, arity } => Rel::LocalGet {
                id: *id,
                arity: *arity,
            },
