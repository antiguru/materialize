...
//!
//! Scalars are opaque (see [`crate::ir`]): the DSL never destructures a
...
pub enum Cond {
    ...
    /// its leaves), via the [`crate::analysis::Monotonic`] analysis. The hook
    ...
    /// a unique key of the bound relation (via the [`crate::analysis::Keys`]
    ...
    /// literal `true` (via [`crate::scalar`]), so a `Filter` by it is the
    /// identity. Vacuously holds for an empty predicate list.
