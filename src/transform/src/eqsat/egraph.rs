...
impl EGraph {
    ...
    fn check_conds(&self, conds: &[Cond], b: &EBindings, an: &Analyses) -> bool {
        ...
        conds.iter().all(|c| match c {
            ...
            Cond::Equiv { pred, rel } => {
                let (Some(p), Some(&id)) = (b.payloads.get(pred), b.rels.get(rel)) else {
                    return false;
                };
                // The predicate payload must be a single equality of two bare
                // column references: `#ca = #cb`. Any other form is left alone.
                let scalars = match p.scalars() {
                    Some(s) if s.len() == 1 => s,
                    _ => return false,
                };
                let mz_expr::MirScalarExpr::CallBinary {
                ...
                let (Some(ca), Some(cb)) = (
                    if let mz_expr::MirScalarExpr::Column(c, _) = &**expr1 {
                        Some(*c)
                    } else {
                        None
                    },
                    if let mz_expr::MirScalarExpr::Column(c, _) = &**expr2 {
                        Some(*c)
                    } else {
                        None
                    },
                ...
                // A self-equality `#a = #a` (ca == cb) is syntactically
                // trivially true and does not require equivalences to drop.
                // We leave it alone here so that constant folding (#a = #a
                // → true) and `drop_true_filter` can handle it cleanly
                // without creating self-referential Filter classes that
                // perturb the equivalences analysis.
                if ca == cb {
                    return false;
                }
                // Check that both columns reduce to the same canonical
                // representative under the relation's equivalences. When the
                // relation is empty (None) every predicate is vacuously true.
                match an.eq.get(&self.find(id)) {
                    Some(Some(ec)) => {
                        let r = ec.reducer();
                        let ea = mz_expr::MirScalarExpr::column(ca);
                        let eb = mz_expr::MirScalarExpr::column(cb);
            ...
            Cond::NotSelfRef { rel } => {
                ...
                // The class is self-referential if any of its e-nodes has a
                // child that also belongs to the same class. When a
                // `drop_equiv_filter` merge creates such a cycle, `merge_filters`
                // would find both the outer and inner Filter in the same class
                // and grow predicate lists without bound. The guard prevents
                // `merge_filters` from firing in that state.
                !self.classes.get(&cid).is_some_and(|nodes| {
                    nodes
                        .iter()
                        .any(|node| node.children().iter().any(|&child| self.find(child) == cid))
                })
            }
        })
    }
