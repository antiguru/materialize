...
fn parser<'a>() -> impl Parser<'a, &'a [Token], Vec<Rule>, TokErr<'a>> {
    ...
    let pat = recursive(|pat| {
        ...
        choice((
            paren, filter, map, project, reduce, negate, threshold, topk, join, wcojoin, union,
            relvar,
        ))
    });
    ...
    let cond = choice((
        ...
        one_ident("unsatisfiable").map(|rel| Cond::Unsatisfiable { rel }),
        kw("join_is_cyclic")
            .ignore_then(just(Token::LParen))
            .then_ignore(just(Token::RParen))
    ...
    let rule = kw("rule")
        ...
        .map(|(((((name, doc), phase), lhs), rhs), conds)| Rule {
            name,
            doc,
