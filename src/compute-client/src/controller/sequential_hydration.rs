...
impl SequentialHydration {
    ...
    pub fn new(dyncfg: Arc<ConfigSet>, metrics: ReplicaMetrics) -> Self {
        Self {
            dyncfg,
            metrics,
    ...
    pub fn absorb_command(&mut self, cmd: ComputeCommand) -> Vec<ComputeCommand> {
    ...
    pub fn observe_response(&mut self, resp: &ComputeResponse) -> Vec<ComputeCommand> {
        let mut commands = Vec::new();
