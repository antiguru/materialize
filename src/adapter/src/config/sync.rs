...
async fn sync_scoped_params(
    ...
    // Feature gate. Scoped (per-cluster / per-replica) overrides are off by
    // default, leaving the environment-wide behavior unchanged. While disabled,
    // evaluate no scoped contexts; if a previously-enabled run left overrides
    // behind, clear them once so resolution falls back to the environment-wide
    // value everywhere.
    if !ENABLE_SCOPED_SYSTEM_PARAMETERS.get(system_config.dyncfgs()) {
        if !catalog.state().scoped_system_parameters().is_empty() {
            client
                .update_scoped_system_parameters(ScopedParameters::default())
                .await;
        }
        return;
    }
...
/// Evaluate the scoped parameters (cluster-coherent and replica-local) for the
/// live objects, optionally restricted to a subset of cluster / replica ids.
///
...
/// configuration / first plan rather than after the next tick. Returns the
